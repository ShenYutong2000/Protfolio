...
export class IndexFloat {
  constructor(gpu) {
    ...
    this._onKey = (event) => {
      if (!['ArrowLeft', 'ArrowRight', 'ArrowUp', 'ArrowDown'].includes(event.key) || event.target?.closest?.('a, button')) return;
      event.preventDefault();
      this.rotY += event.key === 'ArrowRight' ? 0.12 : event.key === 'ArrowLeft' ? -0.12 : 0;
      this.rotX += event.key === 'ArrowDown' ? 0.12 : event.key === 'ArrowUp' ? -0.12 : 0;
  ...
  computeLayout() {
    ...
    for (let i = 0; i < this.gpu.planes.length; i++) {
      ...
      const baseH = Math.min(vh * BASE_HEIGHT_VH, vw * 0.35);
  ...
  _onUp() {
    this.isDragging = false;
    if (this.reducedMotion.matches) { this.velX = 0; this.velY = 0; }
  }
}
